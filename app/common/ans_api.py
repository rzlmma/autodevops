...
class MyInventory(Inventory):
    """ 
    动态生成inventory, inventory主要是获取主机信息，直接解析主机信息，调用inventory.add_host()方法把主机加进去，
    不走inventory自己的解析资源的逻辑
    """
...
class CustomAsi(AnsibleAPI):
    def __init__(self, resource, *args, **kwargs):
        super(CustomAsi, self).__init__(resource, *args, **kwargs)
    ...
    def deal_result(info):
        ...
        处理返回值
        :param info: 
        :return: {"success": ['172.16.0.2', '172.16.0.3'], "failed": [('172.16.0.4', 'error msg')], "unreachable": []} 
        """
        ...
        rest["success"].extend(success.keys())
        rest["failed"].extend(failed.items())
        rest["unreachable"].extend(unreachable.items())
